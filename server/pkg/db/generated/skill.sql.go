...
RETURNING id, workspace_id, name, description, content, config, created_by, created_at, updated_at, is_global, plugin_installation_id
`
...
const getGlobalSkillByName = `-- name: GetGlobalSkillByName :one
SELECT id, workspace_id, name, description, content, config, created_by, created_at, updated_at, is_global, plugin_installation_id FROM skill
...
func (q *Queries) GetGlobalSkillByName(ctx context.Context, name string) (Skill, error) {
	row := q.db.QueryRow(ctx, getGlobalSkillByName, name)
	var i Skill
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const getOldestWorkspaceID = `-- name: GetOldestWorkspaceID :one
...
func (q *Queries) GetOldestWorkspaceID(ctx context.Context) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, getOldestWorkspaceID)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
...
SELECT id, skill_id, path, content, created_at, updated_at, content_encoding, file_mode FROM skill_file
WHERE id = $1
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.content, s.config, s.created_by, s.created_at, s.updated_at, s.is_global, s.plugin_installation_id FROM skill s
JOIN agent_skill ask ON ask.skill_id = s.id
...
const listVisibleSkillSummariesForWorkspace = `-- name: ListVisibleSkillSummariesForWorkspace :many
...
type ListVisibleSkillSummariesForWorkspaceRow struct {
	ID          pgtype.UUID        `json:"id"`
	WorkspaceID pgtype.UUID        `json:"workspace_id"`
	Name        string             `json:"name"`
	Description string             `json:"description"`
...
func (q *Queries) ListVisibleSkillSummariesForWorkspace(ctx context.Context, workspaceID pgtype.UUID) ([]ListVisibleSkillSummariesForWorkspaceRow, error) {
	rows, err := q.db.Query(ctx, listVisibleSkillSummariesForWorkspace, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListVisibleSkillSummariesForWorkspaceRow{}
	for rows.Next() {
		var i ListVisibleSkillSummariesForWorkspaceRow
		if err := rows.Scan(
			&i.ID,
			&i.WorkspaceID,
			&i.Name,
...
ON CONFLICT (skill_id, path) DO UPDATE SET
    ...
    updated_at = now()
RETURNING id, skill_id, path, content, created_at, updated_at, content_encoding, file_mode
`
