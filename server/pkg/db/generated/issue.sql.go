...
WITH current_issue AS (
    SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, working_branch, agent_status, handoff_summary, manual_position_locked, agent_result_at FROM issue WHERE id = $1 AND workspace_id = $2 FOR UPDATE
), top_position AS (
...
UPDATE issue SET
    ...
    position = CASE
        ...
        ELSE position
    END,
    manual_position_locked = manual_position_locked OR $8 IS NOT NULL,
    start_date = $9,
    due_date = $10,
    parent_issue_id = $11,
    project_id = $12,
...
WITH actor_config AS (
    ...
    SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, working_branch, agent_status, handoff_summary, manual_position_locked, agent_result_at FROM issue WHERE id = $1 AND workspace_id = $3 FOR UPDATE
), top_position AS (
    SELECT COALESCE(MIN(i.position) - 1, 0)::double precision AS position
    FROM issue i, current_issue c
    WHERE i.workspace_id = c.workspace_id
