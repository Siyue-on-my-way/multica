...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, working_branch, agent_status, handoff_summary, manual_position_locked, agent_result_at, revision, last_activity_at FROM issue
WHERE workspace_id = $1
...
WITH current_issue AS (
    SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, working_branch, agent_status, handoff_summary, manual_position_locked, agent_result_at, revision, last_activity_at FROM issue WHERE id = $1 AND workspace_id = $2 FOR UPDATE
), top_position AS (
...
WITH actor_config AS (
    SELECT
        set_config(
            ...
            COALESCE($7::text, 'system'),
            true
        ),
        set_config(
            'multica.issue_status_actor_id',
            COALESCE($8::uuid::text, ''),
            true
        )
), candidate AS (
    SELECT
        i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.working_branch, i.agent_status, i.handoff_summary, i.manual_position_locked, i.agent_result_at, i.revision, i.last_activity_at,
        COALESCE($9::text, i.title) AS next_title,
        COALESCE($10::text, i.description) AS next_description,
        COALESCE($11::text, i.status) AS next_status,
        COALESCE($12::text, i.priority) AS next_priority,
        $13::text AS next_assignee_type,
        $14::uuid AS next_assignee_id,
        CASE
            ...
            WHEN i.status IS DISTINCT FROM COALESCE($11::text, i.status)
