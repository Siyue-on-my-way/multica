...
const countCommentsForIssue = `-- name: CountCommentsForIssue :one
SELECT COUNT(*)::bigint FROM comment
WHERE issue_id = $1
  AND workspace_id = $2
`

type CountCommentsForIssueParams struct {
	IssueID     pgtype.UUID `json:"issue_id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) CountCommentsForIssue(ctx context.Context, arg CountCommentsForIssueParams) (int64, error) {
	row := q.db.QueryRow(ctx, countCommentsForIssue, arg.IssueID, arg.WorkspaceID)
	var column_1 int64
...
const getLatestCommentForIssue = `-- name: GetLatestCommentForIssue :one
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at FROM comment
WHERE issue_id = $1
  AND workspace_id = $2
...
type GetLatestCommentForIssueParams struct {
	IssueID     pgtype.UUID `json:"issue_id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) GetLatestCommentForIssue(ctx context.Context, arg GetLatestCommentForIssueParams) (Comment, error) {
	row := q.db.QueryRow(ctx, getLatestCommentForIssue, arg.IssueID, arg.WorkspaceID)
	var i Comment
	err := row.Scan(
		&i.ID,
		&i.IssueID,
...
const listCommentIDsForIssue = `-- name: ListCommentIDsForIssue :many
...
type ListCommentIDsForIssueParams struct {
	IssueID     pgtype.UUID `json:"issue_id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	RowLimit    int32       `json:"row_limit"`
}
...
func (q *Queries) ListCommentIDsForIssue(ctx context.Context, arg ListCommentIDsForIssueParams) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, listCommentIDsForIssue, arg.IssueID, arg.WorkspaceID, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
