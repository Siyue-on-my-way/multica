...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	// Fork: thread the acting identity through to the issue_status_history
	// trigger via the transaction-local settings UpdateIssue consumes.
	if req.Status != nil {
		statusActorType, statusActorID := h.resolveActor(r, userID, workspaceID)
		params.ChangedByType = strToText(statusActorType)
		params.ChangedByID = optionalUUID(statusActorID)
	}
	var issue db.Issue
	attachmentsChanged := false
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	for _, issueID := range req.IssueIDs {
		...
		actorType, actorID := h.resolveActor(r, userID, workspaceID)
		if req.Updates.Status != nil {
			params.ChangedByType = strToText(actorType)
			params.ChangedByID = optionalUUID(actorID)
		}
		var issue db.Issue
		if req.Updates.Description != nil {
