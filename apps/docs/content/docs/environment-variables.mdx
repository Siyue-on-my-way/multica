...
| `MULTICA_LLM_CONFIG_DIR` | empty | Read-only directory containing the fixed per-business YAML configs |
| `MULTICA_LLM_CONFIG_POLL_INTERVAL` | `5s` | How often the server checks the per-business config directory for changes |
