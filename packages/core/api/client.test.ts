...
describe("importSkillArchive", () => {
  it("POSTs multipart form data without a JSON content-type and returns the structured envelope", async () => {
    const skill = {
      id: "skill-1",
      workspace_id: "ws-1",
      is_global: false,
  ...
  it("marks a global import with the global flag", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ status: "created", skill: null }), {
        status: 201,
        headers: { "Content-Type": "application/json" },
      }),
    );
    ...
    await new ApiClient("https://api.example.test").importSkillArchive({
      file,
    ...
    const [, init] = fetchMock.mock.calls[0]!;
    const body = init?.body as FormData;
    expect(body.get("global")).toBe("true");
    expect(body.get("on_conflict")).toBe("overwrite");
  });

  it("passes a same-name conflict envelope through for the dialog to render", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          status: "conflict",
    ...
    const file = new File(["pk"], "review-helper.skill");
    // A 409 conflict rejects through the shared fetch error path; the parsed
    // structured body rides along on the ApiError for the dialog to render.
    await expect(
      new ApiClient("https://api.example.test").importSkillArchive({
        file,
        onConflict: "rename",
      }),
